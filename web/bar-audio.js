...
(function(root){
  ...
  function prepare(){
    try{
      ...
      return loading ||= root.fetch('/assets/bar/audio/bar-sfx-v21.json')
  ...
  async function play(name,hands,delay=0){
    ...
    try{
      ...
      // Reveal cards do not shorten the natural pouring tail.
      let envelope=null;
      if(['success','failure','handRock','handScissors'].includes(name)){
  ...
  function buttonCue(button,text=''){
    ...
    if(label==='换一批'){play('shuffle');return;}
    if(label==='带着这一杯去找ta'){play('detail');return;}
  ...
  root.MamoBarAudio={prepare,play,playDice:()=>play('diceRoll',null,root.matchMedia('(prefers-reduced-motion: reduce)').matches?0:.4),playHands:(me,ta)=>{if([me,ta].every(h=>[0,1,2].includes(h)))return play('handsReveal',[me,ta]);},stop,isStraight,confirmCue,buttonCue};
