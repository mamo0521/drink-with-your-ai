...
(function(root){
  ...
  function prepare(){
    try{
      ...
      return loading ||= root.fetch('/assets/bar/audio/bar-sfx-v13.json')
  ...
  root.MamoBarAudio={prepare,play,playDice:()=>play('diceRoll',null,root.matchMedia('(prefers-reduced-motion: reduce)').matches?0:1.1),playHands:(me,ta)=>{if([me,ta].every(h=>[0,1,2].includes(h)))return play('handsReveal',[me,ta]);},stop,isStraight,confirmCue,buttonCue};
