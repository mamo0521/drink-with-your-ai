...
(function(root){
 ...
 function create(host){
  ...
  function show(){
   ...
   const play=host.hex('转一下',spin);play.classList.add('gg-play');footer.append(play);let spinning=false,raf=0,angle=0,revealTimer=0;stop=()=>{root.MamoBarAudio?.stop('wheelSpin');cancelAnimationFrame(raf);clearTimeout(revealTimer);spinning=false;};
   function spin(){if(spinning)return;root.MamoBarAudio?.play('wheelSpin');clearTimeout(revealTimer);spinning=true;host.lock(true);layout.querySelectorAll('button').forEach(b=>b.disabled=true);play.textContent='转动中';text.textContent='转动中';
