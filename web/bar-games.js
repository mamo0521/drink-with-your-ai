...
(function(root){
 ...
 function create(options={}){
  ...
  const wheelUI=root.MamoBarWheel?.create({reveal:wheelReveal,lock:value=>busy=value,hex,shell(title,subtitle,back){view='wheel';const header=shell(title,subtitle,false,title==='THE GAMES'?setup:back);return {header,content,footer,layout};},tabs(header,show){const tabs=el('div','gg-tabs');const table=btn('赌桌',()=>{root.MamoBarAudio?.play('currency');underlineGroup='tabs';setup();});table.append(el('em','','the table'));const wheel=btn('轮盘',null,'selected');wheel.append(el('em','','the wheel'));wheel.setAttribute('aria-current','page');tabs.append(table,wheel);animateUnderline(tabs,'tabs');header.append(tabs);}});
  ...
  function setup(){
   ...
   const tabs=el('div','gg-tabs');const table=btn('赌桌',null,'selected');table.append(el('em','','the table'));table.setAttribute('aria-current','page');const wheel=btn('轮盘',()=>{root.MamoBarAudio?.play('currency');underlineGroup='tabs';wheelUI?.show();});wheel.append(el('em','','the wheel'));tabs.append(table,wheel);animateUnderline(tabs,'tabs');header.append(tabs);
...
return {open(def={}){menu=def.menu||[];shownTruths=[];loadTruths();wager={type:'drink',drink:null,custom:''};game=null;carriedWheel=null;choice=null;if(screen.hidden){focusBefore=document.activeElement;inerts=[...document.body.children].filter(n=>n!==screen&&n.tagName!=='SCRIPT'&&n.tagName!=='STYLE').map(n=>[n,n.inert]);inerts.forEach(([n])=>n.inert=true);}screen.hidden=false;root.MamoBarMusic?.sync();screen.focus();setup();},hide,resume(payload){if(payload){const p=JSON.parse(JSON.stringify(payload));carriedWheel=p.kind==='wheel'?p:null;if(!carriedWheel){kind=p.kind;wager=p.wager;modes[kind]=p.mode;game=root.MamoBarGameEngine.create({kind,mode:p.mode,wager});Object.assign(game.state,p);}}focusBefore=document.activeElement;screen.hidden=false;root.MamoBarMusic?.sync();inerts=[...document.body.children].filter(n=>n!==screen&&n.tagName!=='SCRIPT'&&n.tagName!=='STYLE').map(n=>[n,n.inert]);inerts.forEach(([n])=>n.inert=true);if(carriedWheel){wheelUI.show();wheelReveal(carriedWheel.result,carriedWheel);}else{table();settlement();}},getState:()=>game?.state};
 }
