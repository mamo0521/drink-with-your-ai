...
(function(root){
 ...
 function create(options={}){
  ...
  function showGuide(){
   ...
   const paragraphs=[['先约好赌注','选喝一杯、真心话、大冒险，或写下自己的赌注。点酒图或左上角换酒按钮，可以重新挑一杯。'],['骰子 · 比大小','单掷：双方各投一次，大者胜。三掷：点三次投掷，双方累计三次点数，总分高者胜。每次上下骰子同时掷出，朝上的一面是本次点数。'],['猜拳','先选石头、剪刀或布，再点「选定出手」，双方手势同时揭晓。一局定输赢，或选择三局两胜：先赢两次的一方获胜，平手后继续出拳。'],['平局，再来一轮','平局卡点「再来一轮」，回到当前玩法，分数清零，按刚才的模式重新开始。'],['赢家来出题','真心话、大冒险在分出胜负后出题。你赢了，就在结算卡写问题或任务；真心话、大冒险都会从各自的题库里随机亮三道给你挑，点「换一批」再换三道，也可以自己写；「亲密题」开关默认关着，打开后题库里「亲密」那一段才会进来（盲品那边也认这个开关）；题库在酒单的 edit 里改；Ta 赢了，就带着结果回聊天，由 Ta 出题。自定义赌注按开局前写好的约定兑现。'],['带着结果找 Ta','结果卡可以先收起，稍后在桌面点「查看结果」。点「带着结果找 Ta」会把结果放进聊天输入框，你可以加一句附言，再按发送。'],['愿赌服输','押好的酒由输家喝。发送结果后按赌约兑现，醉意以酒单数值和吧台上限为准；Ta 在聊天里接着回应。平常点酒由 Ta 决定接下还是推回。']];
