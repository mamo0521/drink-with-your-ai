...
`web/assets/bar/audio/bar-sfx-v19.json` 是本应用使用的音效合辑，包含 Cute & Cozy UI Audio Pack 的已购音效及 reasanka 的 CC0 倒酒声，按各自许可使用，不属于上述文案与图片授权。购买音效允许随应用分发，不允许单独提取后作为音效素材发布。来源和许可见 [音效许可](web/assets/bar/audio/LICENSE.txt)。
